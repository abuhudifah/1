/**
 * components/ProfileSettingsComponent.js — v1.0
 ...
 * 4. عرض معلومات الحساب (الدور، البريد، رقم الحساب)
 * ─────────────────────────────────────────────────────────
 * يُضاف إلى تبويب "الإعدادات" مع الحفاظ على قسم المدير
 */
'use strict';
...
const ProfileSettingsComponent = {
  ...
  _buildInfoCard(user) {
    ...
    const accNum     = AuthService.generateAccountNumber?.(user) || '—';
    ...
    card.innerHTML += `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        ...
        ${this._infoItem('رقم الحساب', `<span dir="ltr" style="font-family:monospace;font-weight:700;color:var(--text-primary);">${escapeHtml(accNum)}</span>`)}
  ...
  async _saveName() {
    ...
    try {
      ...
      // تحديث عنصر الاسم في الهيدر إن وُجد
  ...
  async _saveEquation() {
    ...
    if (isOk(result)) {
      ...
      // إعادة رسم البطاقة لتحديث الحالة
  ...
  async _disableQuickLogin() {
    ...
    if (isOk(result)) {
      // إزالة Device Token من sessionStorage
      try {
        const uid = AuthService.getCurrentUserId();
...
console.log('✅ ProfileSettingsComponent.js v1.0 محمّل');
