/**
 * components/LoginComponent.js — v3.1
 * نظام أبو حذيفة
 *
 * الإصلاح في v3.1:
 * ✅ quickEnabled يتحقق من localStorage (يبقى بين الجلسات)
 *    وليس فقط sessionStorage (يُمسح عند إغلاق المتصفح)
 *    → يحل السبب الثاني لفشل الدخول السريع
 */
'use strict';

// ─── دالة مساعدة: هل يوجد دخول سريع محفوظ على هذا الجهاز؟ ───
function _hasAnyQuickLogin() {
  try {
    if (sessionStorage.getItem(SECURITY_CONFIG.DEVICE_TOKEN_KEY)) return true;
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k && k.startsWith('ahu_quick_')) {
...
const LoginComponent = {
  ...
  render(container, onSuccess) {
    ...
    // ✅ الإصلاح: استخدام _hasAnyQuickLogin بدلاً من sessionStorage فقط
  ...
  _injectCalcStyles() {
    ...
    style.textContent = `
      .calc-card {
        ...
        box-shadow: 0 32px 80px rgba(15,23,42,0.18), inset 0 1px 0 rgba(255,255,255,0.6);
      ...
      .calc-display {
        ...
        text-align: left; direction: ltr; min-height: 90px;
      ...
      .login-card {
        background: rgba(255,255,255,0.92); backdrop-filter: blur(28px); -webkit-backdrop-filter: blur(28px);
        border: 1px solid rgba(15,23,42,0.10); border-radius: 28px; padding: 28px 24px 24px;
        box-shadow: 0 32px 80px rgba(15,23,42,0.18), inset 0 1px 0 rgba(255,255,255,0.6);
      }
      .login-input {
        width:100%; padding:12px 16px; border-radius:14px; background:rgba(15,23,42,0.05);
        border:1.5px solid rgba(15,23,42,0.12); color:#0f172a; font-size:0.92rem;
      ...
      .login-submit {
        ...
        box-shadow:0 4px 20px rgba(37,99,235,0.35); transition:box-shadow 150ms, transform 150ms;
      }
      ...
      body.dark-mode .login-card  { background:rgba(30,41,59,0.94); border-color:rgba(248,250,252,0.08); box-shadow:0 32px 80px rgba(0,0,0,0.50), inset 0 1px 0 rgba(255,255,255,0.04); }
      body.dark-mode .login-input { background:rgba(15,23,42,0.50); border-color:rgba(248,250,252,0.12); color:#f1f5f9; }
      body.dark-mode .login-input:focus { border-color:rgba(59,130,246,0.60); box-shadow:0 0 0 3px rgba(59,130,246,0.18); background:rgba(30,41,59,0.80); }

      .login-page-wrap {
        min-height:100vh; display:flex; align-items:center; justify-content:center;
        padding:20px; position:relative; overflow:hidden; transition:background 0.4s;
      }
      ...
      .quick-hint { text-align:center;font-size:0.75rem;color:#059669;margin-bottom:12px;display:flex;align-items:center;justify-content:center;gap:4px; }
  ...
  _buildLoginCard() {
    ...
    emailInput.id='login-email'; emailInput.type='email'; emailInput.autocomplete='email';
    emailInput.placeholder='name@example.com'; emailInput.dir='ltr'; emailInput.className='login-input';
    ...
    passInput.id='login-password'; passInput.type='password'; passInput.autocomplete='current-password';
    passInput.placeholder='••••••••'; passInput.className='login-input'; passInput.style.paddingLeft='44px';
    const eyeBtn = document.createElement('button');
    eyeBtn.type='button';
    eyeBtn.style.cssText='position:absolute;left:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--text-muted,#94a3b8);padding:4px;font-size:1rem;transition:color 150ms;';
    eyeBtn.innerHTML='👁'; eyeBtn.setAttribute('aria-label','إظهار/إخفاء كلمة المرور');
    ...
    passRow.appendChild(passInput); passRow.appendChild(eyeBtn);
    passWrap.appendChild(passLabel); passWrap.appendChild(passRow);
    ...
    [emailInput,passInput].forEach(inp=>inp.addEventListener('keydown',e=>{ if(e.key==='Enter') this._handleLogin(emailInput,passInput,submitBtn,errEl); }));
    card.appendChild(submitBtn);

    // قسم إعداد الدخول السريع (يظهر بعد تسجيل الدخول التقليدي)
  ...
  async _tryQuickLogin(equation) {
    ...
    console.log('[LoginComponent] محاولة دخول سريع...');
    ...
    if (isOk(res)) {
      ...
      // لا نُظهر خطأ إذا كان المستخدم يحسب فقط (يتابع الكتابة)
      if (res.error && !res.error.includes('معادلة فارغة')) {
        console.log('[LoginComponent] فشل دخول سريع:', res.error);
      }
    }
  },
  ...
  async _handleLogin(emailInput, passInput, btn, errEl) {
    ...
    if (isOk(result)) {
      ...
      // اعرض إعداد الدخول السريع فقط إذا لم يكن مفعّلاً بالفعل
      if (qs && !profile.quick_equation_hash) {
  ...
  _bindQuickSetup(profile) {
    ...
    eqInput?.addEventListener('input', ()=>{
      ...
      try {
        ...
        preview.textContent=`النتيجة: ${val}`; preview.style.color='rgba(5,150,105,0.8)';
      } catch {
    ...
    saveBtn?.addEventListener('click', async ()=>{
      ...
      if (isOk(res)) {
        ...
        // ✅ تحديث quickEnabled في الحالة (للجلسة الحالية)
...
console.log('✅ LoginComponent v3.1 — إصلاح quickEnabled (localStorage + sessionStorage)');
